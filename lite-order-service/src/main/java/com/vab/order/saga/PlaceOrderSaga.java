...
public class PlaceOrderSaga implements SimpleSaga<PlaceOrderSagaData> {
    ...
    // LITE: FULFILMENT_CHANNEL removed — fulfilment is a local stub (fulfilment-service dropped).
    ...
    // ── Fulfilment: LITE local stub, no participant endpoint (see fulfilStub) ───
    ...
    public PlaceOrderSaga(@Lazy OrderCommandService orderCommandService) {
        ...
        this.sagaDefinition = step()
            ...
            .step()
                // 11 — LITE fulfil: local stub (fulfilment-service dropped). Forward-only;
                //      skipped while forward-recovering. Stamps a synthetic delivery ref so
                //      finalize can COMPLETE the order on the happy path.
                .invokeLocal(this::fulfilStub)
            .step()
    ...
    // ── Step 11: LITE fulfil stub (fulfilment-service dropped) ──────────────────

    /**
     * LITE replacement for the fulfilment participant. Forward-only local step: on
     * the happy path it stamps a synthetic delivery ref so {@link #finalizeOrder}
     * COMPLETES the order. Skipped while forward-recovering (mirrors the old
     * {@code shouldFulfil} gate). The full build sent {@code FulfilOrderCommand} to
     * fulfilment-service, which called OTT — both removed in Lite.
     */
    private void fulfilStub(PlaceOrderSagaData data) {
        if (data.isForwardRecover()) {
            log.info("Saga fulfil (lite stub) skipped — forward-recovering: orderId={}", data.getOrderId());
            return;
        }
        String ref = "lite-" + data.getOrderId();
        data.setFulfilmentRef(ref);
        data.setExternalRef(ref);
        log.info("Saga fulfil (lite stub) OK: orderId={}, fulfilmentRef={}", data.getOrderId(), ref);
    }
