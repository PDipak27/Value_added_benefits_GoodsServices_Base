...
## DD-27 — OTT entitlements externalized to a standalone `ott-service`; DIGITAL_SUBSCRIPTION provisioning failures park the order in `FULFILMENT_FAILED` for admin-driven re-drive  *(extends DD-22/DD-26 for the DIGITAL_SUBSCRIPTION type only)*

**Problem/context:** DIGITAL_SUBSCRIPTION fulfilment was an in-process stub inside fulfilment-service that fabricated an `externalRef "OTT-…"`. We want the entitlement to live in a *real external system*. So provisioning moves to a standalone **`ott-service`** with its own database. Deliberately kept simple for the learning goal: **no OIDC, no API gateway** — fulfilment-service calls OTT directly. This raises three questions answered here: sync vs async call, how to treat OTT failures inside the saga, and how an order recovers once a provisioning error has been fixed.

**Topology / placement (the OTT call lives in fulfilment-service, not order-service):**
```
order-service ──Tram cmd (fulfilment.Fulfil.v1)──▶ fulfilment-service
                                                        │  (DIGITAL_SUBSCRIPTION branch only)
                                                        ▼  sync HTTP POST /ott/v1/entitlements
                                                   ott-service ──▶ ott DB
                                                        │  201 {externalRef} / 422 / 503
                                                        ▼
        fulfilment-service maps result → Tram reply: OrderFulfilled | OrderProvisioningFailed

  Admin re-drive of a parked order RE-SENDS the same Tram fulfil command (no saga):
  order-service ──Tram cmd (fulfilment.Fulfil.v1), replyTo=order-service──▶ fulfilment-service ──▶ ott-service
                ◀── Tram reply (OrderFulfilled | OrderProvisioningFailed, carries orderId) ──
```
fulfilment-service is already the per-type dispatch point, so the blast radius is one branch. OTT has **no Tram, no messaging** — it is a dumb provisioning box. PHYSICAL_GOOD / SOFTWARE_LICENSE are untouched.

**Sync REST, with bounded in-call retries (not async):** the saga step is *already* an async command/reply, so making the OTT call a blocking REST call **inside the participant** keeps a single round-trip and avoids a second messaging hop with callback correlation. fulfilment-service performs a small bounded **in-call retry** (~3 attempts, short backoff) for transient errors (connection refused, timeout, 5xx); a 422 is never retried.

**`ott-service` surface:** `POST /ott/v1/entitlements {orderId, subscriberId, offerCode}` → `201 {externalRef, status:"ACTIVE"}`. Table `entitlements(external_ref PK, order_id UNIQUE, subscriber_id, offer_code, status, provisioned_at)`; `externalRef = "OTT-<uuid8>"`. **Idempotency keyed on `orderId`:** a repeat for an order that already has a row returns **200 with the existing `externalRef`** (never a duplicate row, never a 409). This is load-bearing — it makes both Tram at-least-once redelivery *and* the admin re-drive (below) safe to repeat.

**Failure handling — one non-terminal stuck state, no auto-retry tier:** once the in-call retries are exhausted, **any** remaining provisioning failure — transient (5xx/timeout) *or* non-transient (422) alike — makes fulfilment reply with a **success-outcome participant event `OrderProvisioningFailed(reason, detail)`**. This is a *new* reply in `events.fulfilment`, deliberately **distinct from the DD-26 `OrderFulfilmentFailed`**: that one drives forward-recovery (refund/reverse → `CANCELLED_REFUNDED`); this one means "do not unwind — park". The order moves to the non-terminal status **`FULFILMENT_FAILED`**, emits a domain event, and notification-service alerts the admin (*"fulfilments failing on provisioning errors"*). There is **no order-level transient/non-transient classification, no scheduled sweeper, no time-boxed retry window, and no ShedLock.**

> *Rejected alternative — an auto-retry tier.* We designed and discarded a richer model (`PROVISIONING_RETRY` swept every 30 min for 6 h, then escalating to a manual state, with a separate `CANCELLED_REFUNDED` abandon path). It required a scheduled sweeper, a **multi-instance coordination** concern (ShedLock or `SELECT … FOR UPDATE SKIP LOCKED`, since several service instances would each fire the `@Scheduled` job), and transient-vs-non-transient branching — significant machinery for a case that, for an external-integration outage, is genuinely an **ops ticket**. The simpler "fail to one state, notify a human, let them fix and re-drive" wins. (Correctness note for the record: even the rejected sweeper would have been *safe* under multiple instances — OTT idempotency on `orderId` plus the Order `@Version` optimistic lock prevent double-provisioning or double-completion — so coordination there would have been an efficiency concern, not a correctness one.)

**Crucial reply semantics (the trap that a SUCCESS reply hides):** the SUCCESS *outcome* means only **"do not compensate"** — it stops Eventuate from rolling back the already-settled charge. It does **not** mean "complete the order." The saga's **finalize step branches on the reply content**: `OrderFulfilled(externalRef)` → **COMPLETED**; `OrderProvisioningFailed` → set a `provisioningFailed` flag → finalize parks at **FULFILMENT_FAILED** and the saga ends there. (The flag is kept *separate* from DD-26's `forwardRecover`, so the refund/reverse/release steps stay skipped — the charge is not unwound.) The completion transition must therefore be **conditional**, never an unconditional "mark completed" step that always runs after fulfil — otherwise a success-outcome *failure* reply would wrongly complete the order.

**Recovery re-sends the fulfil command, but is *not* a resumed saga (`SagaManager` has no `retry()`):** verified against `eventuate-tram-sagas-orchestration` **0.26.0** — `SagaManager` exposes only `create(...)` (×3) and `subscribeToReplyChannel()`; the impl adds setters and `handleMessage(Message)`. **There is no `retry()` / resume primitive.** A saga advances *only* when a correlated reply reaches `handleMessage`; once fulfilment replies, the step is resolved and the instance is done — it cannot be "continued" later. So `FULFILMENT_FAILED` is an **order-aggregate state, not a paused saga**. Crucially, *resuming a saga* and *re-sending a command* are different things: recovery re-drives the single incomplete step by **re-sending the same `FulfilOrderCommand` as a plain Tram command** (not via the orchestrator), so it re-enters fulfilment-service's `fulfil()` handler and runs the *same* product-type dispatch against the now-healthy OTT — one fulfilment entry point, the existing command + reply contracts reused, no parallel REST surface. **No new saga** is introduced.

> Two consequences of staying on the messaging path. **(1) Correlation:** a live saga keeps `orderId` in saga-data, but a reply that arrives outside a saga has nowhere to look it up — and `OrderFulfilled` originally carried no `orderId`. So the two re-drive replies (`OrderFulfilled`, `OrderProvisioningFailed`) now **carry `orderId`** and are self-correlating; order-service runs a small reply consumer (`FulfilmentReDrive`) subscribed to its own reply channel that applies the outcome to the parked aggregate. **(2) Asynchrony:** `retry-fulfilment` returns **202** and the order completes (or re-parks) when the reply lands — versus a blocking REST call that returns the outcome inline. We accept the async admin UX (poll the order status) to keep a single fulfilment code path and avoid a second, divergent provisioning surface.

After fixing the external service the admin uses:
- `POST /v1/orders/{id}/retry-fulfilment` → order-service re-sends the `FulfilOrderCommand` (replyTo = its own channel) → `fulfil()` dispatches to OTT → reply lands on the consumer → on `OrderFulfilled` the order applies **COMPLETED** with the returned `externalRef`; on `OrderProvisioningFailed` it **stays parked** and re-alerts. (OTT idempotency on `orderId` makes a previously half-succeeded call safe — it returns the existing `externalRef`. The apply step is also idempotent: a reply for an order no longer parked is a no-op.)
- `POST /v1/orders/{id}/complete-fulfilment {externalRef}` → manual override for when the admin provisioned by hand in OTT's console; order-service applies **COMPLETED** directly with the supplied `externalRef` (pure reconciliation, no OTT call).

Both admin endpoints return **409** if the order is not currently in `FULFILMENT_FAILED`.

*(An `abandon → CANCELLED_REFUNDED` escape hatch from `FULFILMENT_FAILED` was discussed and is **deferred** — not part of this DD.)*

**State machine (DIGITAL_SUBSCRIPTION only):**
```
CONFIRMED ──online fulfil (in-call ~3 sync retries)──┐
   ok                       → COMPLETED
   any failure remaining    → FULFILMENT_FAILED   (emit event → notify admin)

FULFILMENT_FAILED  (admin fixes external OTT, then…)
   retry-fulfilment         → re-drive → OrderFulfilled → COMPLETED
   complete-fulfilment      → COMPLETED (admin-supplied externalRef)
```

**Scope / divergence from DD-26 (deliberate):** this applies **only to DIGITAL_SUBSCRIPTION/OTT**. PHYSICAL_GOOD and SOFTWARE_LICENSE fulfilment failures **keep DD-26** (forward-recovery → `CANCELLED_REFUNDED`). Rationale: an external-integration provisioning fault is an operational issue an admin should triage and fix (then re-drive), not silently auto-refund.

**Naming — two distinct objects, deliberately different names:** the **participant reply** fulfilment-service sends to the saga is `events.fulfilment.OrderProvisioningFailed` (drives the finalize park branch); the **order domain event** emitted when the aggregate enters `FULFILMENT_FAILED` is `events.order.OrderFulfilmentFailed` (published for the read model / notification). They were given *different* names — not just different packages — precisely because `events.fulfilment` already has an `OrderFulfilmentFailed` with the opposite meaning (DD-26 forward-recovery); reusing that name for the park reply would be a correctness trap.

**Demo triggers:** `offerCode` containing **`OTTDOWN`** → ott-service returns `503` (→ after in-call retries → `FULFILMENT_FAILED`; "fix OTT" then `retry-fulfilment` → `COMPLETED`); `offerCode` containing **`OTTBAD`** → `422` (→ `FULFILMENT_FAILED` directly). Existing `FAIL` semantics for physical/license are unchanged.

**What this changed (as implemented):**
- **new module `ott-service`:** Spring Boot REST + own `ott` schema (no Tram); `POST /ott/v1/entitlements`; idempotent on `orderId` (`order_id UNIQUE`); `externalRef "OTT-<uuid8>"`; demo triggers `OTTDOWN`→503, `OTTBAD`→422. Port 8087.
- **shared-events:** new participant reply `events.fulfilment.OrderProvisioningFailed`; new domain event `events.order.OrderFulfilmentFailed`.
- **fulfilment-service:** new `OttClient` (sync REST, bounded in-call retries; 5xx/timeout retried, 422 not) + `OttProvisioningService`; DIGITAL_SUBSCRIPTION branch replies `withSuccess(OrderProvisioningFailed)` on failure (never `withFailure`). The `fulfil()` command handler is the **single** provisioning entry point — used by both the saga and the admin re-drive (no extra REST surface). The success/park replies now carry `orderId` for re-drive correlation.
- **order-service:** `OrderStatus` adds the **non-terminal** `FULFILMENT_FAILED`; `Order` gains `last_attempt_at` (reuses `failed_step`/`failure_reason`) and a `fulfilmentFailed()` transition (V5 migration); the saga adds `.withReply(OrderProvisioningFailed)` + handler setting a `provisioningFailed` flag, and finalize branches park→`FULFILMENT_FAILED`; new `FulfilmentReDrive` (`CommandProducer` send + a `MessageConsumer` reply consumer on its own channel); `OrderCommandService` adds `fulfilmentFailed` / `retryFulfilment` (re-sends the Tram command) / idempotent `completeFromReDrive` / `parkFromReDrive` (reply-applied) / `completeFulfilment` (manual override); `OrderCommandController` adds `POST /{id}/retry-fulfilment` (202) and `POST /{id}/complete-fulfilment` (409 if not parked); projector maps `FULFILMENT_FAILED`.
- **notification:** new `ORDER_FULFILMENT_FAILED` type → backoffice/admin alert (EMAIL channel). It is **addressed to the ops desk** (`notification.admin-recipient`), not the subscriber — `NotificationType.isBackoffice()` selects the recipient and the resolved address is persisted on `delivery_log` (V2 migration) for audit.
- **demo seed data:** two always-failing DIGITAL_SUBSCRIPTION offers — `OTT_OTTDOWN_1M` (503) and `OTT_OTTBAD_1M` (422) — are seeded in catalog (`CatalogSeeder`) with matching inventory rows (inventory V5 migration) so reserve succeeds and the saga actually reaches the OTT step and parks (without the rows it would fail `ITEM_NOT_FOUND` and never exercise the park).
- **tests:** unit coverage for the admin alert (`NotificationEventConsumerTest`); **Testcontainers** integration tests against a real Postgres — `NotificationDeliveryIntegrationTest` (provisioning-failed → EMAIL to ops desk persisted) and order-service `OrderPersistenceIntegrationTest` (place + `FULFILMENT_FAILED` park + idempotency round-trip). A black-box **`e2e-tests`** module (REST Assured + Awaitility, opt-in `-Pe2e`, `*E2E` classes) drives the live stack end-to-end across catalog, OTT, the order happy/failure/cancel paths, and the DD-27 park + admin re-drive.
- **Eventuate semantics reconfirmed:** no `SagaManager.retry()` / resume — recovery **re-sends the fulfil command** as a plain Tram command/reply (re-entering `fulfil()`), which is *not* a resumed saga and needs no new saga.

---

## DD-28 — Operational hardening: uniform error responses, bounded failure-detail, eager Jackson registration, per-service code logging  *(cross-cutting; no flow changes)*

**Context:** Exercising the DD-27 OTT park flow surfaced a few cross-cutting defects/gaps rather than flow-specific ones. None changes the saga model; all make the system safer to operate.

**1. A provider stack trace overflowed `saga_instance.saga_data_json`.** When `ott-service` returned an error whose body carried a stack trace, `OttClient` copied the entire HTTP body into the failure `detail` (via `RestClientResponseException.getMessage()`); it flowed through `OrderProvisioningFailed` into the saga and was persisted to Eventuate's `saga_data_json` column (`VARCHAR(1000)`) — overflowing it and aborting the saga, besides being log noise. **Fixed in three layers:** (a) `OttClient` now builds `detail` from the **status line** plus the provider's **`ProblemDetail.detail`** *only* when the response is `application/problem+json` (content-type-gated; a malformed body or any other type degrades to status-only), length-capped at 180 — never the raw body; (b) the saga **caps** `provisioningFailureReason`/`cancelReason` at 256 chars before they enter `PlaceOrderSagaData`, defending the column regardless of source; (c) `ott-service` sets `server.error.include-stacktrace|message|exception = never|never|false` so even the default error path carries no internals.

**2. Uniform error responses via `@RestControllerAdvice` (RFC-7807 `ProblemDetail`).** The three HTTP services — `ott-service`, `order-service`, `catalog-service` — get an identical `GlobalExceptionHandler` extending `ResponseEntityExceptionHandler`: a deliberate `ResponseStatusException` keeps its status + reason (order's `409` conflicts, OTT's `422`/`503`); `DataAccessException` (SQL/constraint/connection) → `503`; anything else → `500`; and Spring's own client errors (unreadable body, validation, `404`/`405`) keep their correct `4xx` instead of being swallowed to `500`. Stack traces are logged server-side only, never in the body. The other five apps (saga participants / consumer / gateway) expose no controllers, so they get none.

**3. Eager `JavaTimeModule` registration on pure consumers.** Eventuate's static `JSonMapper` omits `JavaTimeModule`; `Instant`-bearing events register it from a class static-block that fires only on *instantiation*. A *pure* reply/event consumer that touches the type only as a `.class` literal (order-service's saga consuming `InventoryReserved.reservedUntil`; notification-service) never triggers that hook, so the first `Instant` payload failed to deserialize and silently stalled the consumer. Fix: call `EventuateJackson.register()` eagerly in `main()` of those services.

**4. Per-service code-only logging.** Each app adds a `logback-spring.xml` routing **only `com.vab.*`** statements to its own `vabags-<svc>.log` (default dir `…/source/applogs`, overridable with `-DAPP_LOG_DIR`), rolling strictly at 1 MB into numbered archives (`FixedWindowRollingPolicy` + `SizeBasedTriggeringPolicy`); framework/3rd-party logs stay on the console. Keeps app logs separable from framework noise without a shared-file/multi-JVM contention problem.
...
- **order-service** becomes a servlet resource server; `POST /v1/orders` drops `subscriberId` from its body DTO and reads it from the JWT, and the query endpoints drop `?subscriberId=` (now "my orders" / "my benefits" scoped to the token subject).
- **Object-level authorization (§A-3 hardening, fixes the initial IDOR/BOLA gap):** the by-id endpoints — `GET /v1/orders/{id}`, `.../timeline`, `POST /v1/orders/{id}/cancel` — now enforce **ownership**: the order's `subscriberId` must equal the caller's, unless the caller holds `vab-admin`. A non-owner receives **`404`** (not `403`) so the endpoint never reveals whether the order exists. (`isAdmin(jwt)` reads `realm_access.roles`; the check is load-then-compare, returning 404 on mismatch — equivalent to a scoped query but keeping the read-your-writes write-store fallback.)
